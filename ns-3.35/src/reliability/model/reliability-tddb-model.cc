...
{
  static TypeId tid =
      TypeId ("ns3::ReliabilityTDDBModel")
          .SetParent<ReliabilityModel> ()
          .SetGroupName ("Reliability")
          .AddConstructor<ReliabilityTDDBModel> ()
          .AddAttribute (
              "A", "Parameter A.", DoubleValue (1.0),
              MakeDoubleAccessor (&ReliabilityTDDBModel::SetA, &ReliabilityTDDBModel::GetA),
              MakeDoubleChecker<double> ())
          .AddAttribute (
              "B", "Parameter B.", DoubleValue (0.5),
              MakeDoubleAccessor (&ReliabilityTDDBModel::SetB, &ReliabilityTDDBModel::GetB),
              MakeDoubleChecker<double> ())
          .AddAttribute (
              "area", "Area.", DoubleValue (1.0),
              MakeDoubleAccessor (&ReliabilityTDDBModel::SetArea, &ReliabilityTDDBModel::GetArea),
              MakeDoubleChecker<double> ())
          .AddTraceSource ("Reliability", "Reliability of the device.",
...
ReliabilityTDDBModel::ReliabilityTDDBModel ()
{
  ...
  m_reliability = 1.0;
  m_reliabilityUpdateEvent =
      Simulator::Schedule (m_reliabilityUpdateInterval, &ReliabilityModel::UpdateReliability, this);
...
double
ReliabilityTDDBModel::scale_par (double T, double V, double offset_a, double mult_a, double tau_a,
                                 double tauvolt_a) const
{
  double value;
...
double
ReliabilityTDDBModel::shape_par (double T, double V, double mult_b, double tau_b, double offset_b,
                                 double multvolt_b) const
{
  double value;
...
ReliabilityTDDBModel::UpdateReliability ()
{
  ...
  scale_parameter =
      365 * 24 * 60 * 60 * scale_par (temperature, voltage, offset_a, mult_a, tau_a, tauvolt_a);
  shape_parameter = shape_par (temperature, voltage, mult_b, tau_b, offset_b, multvolt_b);

  int i, j;
  double u, v, t_0, g_value, exponential_value, pdf_u_value, pdf_v_value, joint_pdf_value,
      integral_subdomain, g_value_prec, exponential_value_prec, integral_subdomain_prec, damage;
  double Rc = 0, Rc_prec = 0;
      ...
      for (j = 0; j < v_num_step; j++)
        {
          ...
          g_value = g (u + 0.5 * subdomain_step_u, v + 0.5 * subdomain_step_v, t_0, scale_parameter,
                       shape_parameter);
          ...
          if (LI_index > 1)
            {
              g_value_prec = g (u + 0.5 * subdomain_step_u, v + 0.5 * subdomain_step_v,
                                t_0 - delta_LI, scale_parameter, shape_parameter);
