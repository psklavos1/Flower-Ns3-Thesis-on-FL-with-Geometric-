...
Server::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::Server")
          ...
          .AddAttribute ("ModelType", "The name of the modelType used for training",
                         TypeId::ATTR_SGC, StringValue ("MNIST"),
                         MakeStringAccessor (&Server::m_modelType), MakeStringChecker ())
          .AddAttribute ("DeviceType", "The Device type used for machine learnign",
                         TypeId::ATTR_SGC, StringValue ("400"),
                         MakeStringAccessor (&Server::m_deviceType), MakeStringChecker ())
          ...
          .AddAttribute ("Async", "Run as async client", TypeId::ATTR_SGC, BooleanValue (false),
                         MakeBooleanAccessor (&Server::m_bAsync), MakeBooleanChecker ())
          .AddAttribute ("BytesModel", "Number of bytes in model", TypeId::ATTR_SGC,
                         UintegerValue (), MakeUintegerAccessor (&Server::m_bytesModel),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("TimeOffset", "Time offset to add to simulation time reported",
                         TypeId::ATTR_SGC, TimeValue (), MakeTimeAccessor (&Server::m_timeOffset),
                         MakeTimeChecker ());
...
Server::Server ()
    : m_packetSize (0), m_sendEvent (), m_bytesModel (0), m_bAsync (false), m_flwrProvider (nullptr)
{
  m_socket = 0;
}
          ...
          if (m_bAsync)
            {
              ...
              if (m_flwrProvider)
                {
                  FlwrProvider::AsyncMessage message;

                  message.id = m_clientSessionManager->ResolveToIdFromServer (socket);
                  message.endTime = endUplink;
                  message.startTime = beginDownlink;
                  message.throughput =
                      itr->second->m_bytesReceived * 8.0 / 1000.0 / ((endUplink - beginUplink));

                  m_flwrProvider->send (&message);
...
void
Server::StartSendingModel (Ptr<Socket> socket)
{
  auto itr = m_socketList.find (socket);
