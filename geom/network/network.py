...
class Network(object):
    ...
    def __init__(self, config: Dict[str, Any], client_cfg: Dict[str, Any]):
        ...
        self.csma_datarate = config.csma_datarate
    ...
    def connect(self):
        try:
            ...
            print(
                f"ConnectionRefusedError: Failed to connect to NS3 at {self.tcp_ip}:{self.port} - {e}"
            )
            raise
        except Exception as e:
            print(
                f"Unexpected error occurred while connecting to NS3 at {self.tcp_ip}:{self.port} - {e}"
            )
            raise
