...
class FedAvgWithGeometric(FedAvg):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.data_manager = DataManager(log_dir=str(root_dir) + "/csv_logs/new")
    ...
    def aggregate_evaluate(
        ...
        for client_proxy, eval_res in results:
            ...
            if epoch_data and batch_data:  # client logging results
                ...
                ns3_data = {
                    "router_distace": self.ns3_res[
                        self.monitor.get_index(client_proxy.cid)
                    ]["routerDistance"],
                    "throughput": self.ns3_res[
                        self.monitor.get_index(client_proxy.cid)
                    ]["throughput"],
                }
                self.data_manager.append_epoch_data_to_last_row(
                    ns3_data,
                    client_id=client_id,
                )
...
def get_evaluate_fn(testset, dataset, ann):
    ...
    def evaluate_fn(server_round: int, parameters, config):
        ...
        to_plot_model = True if server_round == 1 else False
        model = CustomModel(ds_name=dataset, ann_name=ann,plot_model=to_plot_model)
