...
namespace ns3 {
void
FlwrProvider::waitForConnection ()
{
...
FlwrProvider::COMMAND::Type
FlwrProvider::recv (std::map<int, std::shared_ptr<ClientSession>> &packetsReceived)
{
  COMMAND c;
  ...
  for (auto it = packetsReceived.begin (); it != packetsReceived.end (); it++, i++)
    {
      ...
      if (sizeof (temp) != read (m_new_socket, (char *) &temp, sizeof (temp)))
        {
          NS_LOG_UNCOND ("Invalid length received");
          return COMMAND::Type::EXIT;
        }
...
void
FlwrProvider::send (AsyncMessage *pMessage)
{
  COMMAND r;
