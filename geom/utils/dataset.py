...
def _generate_high_bias_class_percentages(num_classes, seed=None):
    ...
    # Introduce variability in the percentage assignment
    percentages = np.random.uniform(0.01, 0.2, num_classes)

    # Select a few classes to have significantly higher percentages
    ...
    for focus_class in focus_classes:
        percentages[focus_class] = np.random.uniform(
            0.2, 0.5
        )  # Higher bounds for more non-IID distribution

    # Normalize percentages to ensure they sum up to 1
