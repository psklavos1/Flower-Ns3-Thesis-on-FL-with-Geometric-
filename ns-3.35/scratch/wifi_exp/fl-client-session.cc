...
namespace ns3 {
    ...
    ClientSession::ClientSession(int clientID_, double radius_, double theta_, char* dataRate_) :
            m_client(nullptr), m_radius(radius_), m_theta(theta_), m_dataRate(dataRate_), m_clientID(clientID_), m_cycle(0), m_inRound(true),
            m_dropOut(false)
    {
    ...
    char* ClientSession::GetDataRate()
    {
        return m_dataRate;
    }
